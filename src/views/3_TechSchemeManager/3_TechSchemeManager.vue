                ...
                <div class="row-actions">
                  ...
                  >
                    <i
                      ...
                      :class="{ spinning: regeneratingId === item.id }"
                    ></i>
                  </button>
                </div>
              </td>
          ...
          <div class="modal-body">
            <section class="form-card">
              ...
              <div class="grid-two">
                ...
                <label class="field"><span>工程地点</span><input v-model.trim="formModel.projectAddress" type="text" /></label>
                <label class="field"><span>鉴定类型</span><input v-model.trim="formModel.appraisalType" type="text" /></label>
            ...
            <section class="form-card">
              ...
              <div class="grid-one body-fields">
                <label class="field"><span>工程概况</span><textarea v-model.trim="formModel.overview" rows="4"></textarea></label>
                <label class="field"><span>鉴定目的及范围</span><textarea v-model.trim="formModel.purposeScope" rows="4"></textarea></label>
                <label class="field"><span>检测鉴定依据</span><textarea v-model.trim="formModel.basis" rows="4"></textarea></label>
                <label class="field"><span>检测数量</span><textarea v-model.trim="formModel.quantity" rows="3"></textarea></label>
                <label class="field"><span>检测鉴定仪器设备</span><textarea v-model.trim="formModel.equipment" rows="3"></textarea></label>
                <label class="field"><span>检测鉴定技术方法</span><textarea v-model.trim="formModel.method" rows="4"></textarea></label>
                <label class="field"><span>鉴定报告编写</span><textarea v-model.trim="formModel.reportWriting" rows="4"></textarea></label>
                <label class="field"><span>鉴定及检测质量保证措施</span><textarea v-model.trim="formModel.qualityMeasure" rows="4"></textarea></label>
                <label class="field"><span>安全作业保证措施</span><textarea v-model.trim="formModel.safetyMeasure" rows="4"></textarea></label>
                <label class="field"><span>需委托方配合事项及要求</span><textarea v-model.trim="formModel.cooperation" rows="4"></textarea></label>
              </div>
            </section>
          </div>
          ...
          <div class="modal-body preview-body">
            <div class="document-paper">
              <section class="cover-section">
                ...
                <p><strong>编写：</strong>{{ previewScheme.writer }}　<strong>校核：</strong>{{ previewScheme.reviewer }}　<strong>批准：</strong>{{ previewScheme.approver }}</p>
...
function useTechSchemeManager() {
  ...
  const filteredRows = computed(() => {
    ...
    return schemeList.value.filter((item) => {
      ...
      return [item.schemeName, item.projectName, item.schemeNo].join(' ').toLowerCase().includes(keyword)
    })
  })
...
.action-bar {
  ...
  border-radius: 18px !important; /* 对齐大圆角 */
...
.btn-primary { background: #2a3f7a !important; color: #fff !important; }
.btn-primary:hover { background: #3e5a9e !important; transform: translateY(-1px); }
.btn-danger { background: #f56c6c !important; color: #fff !important; }
.btn-danger:hover { background: #f78989 !important; transform: translateY(-1px); }
...
.panel__title {
  display: none !important; /* 隐藏多余的内部大标题，模块四没有这个 */
}
...
table {
  ...
  font-size: 13px !important; /* 👈 改为 13px */
  color: #334155 !important;  /* 👈 颜色加深，对齐第四模块 */
...
th, td {
  padding: 14px 12px !important; /* 👈 左右间距加大到 12px */
  border-bottom: 1px solid #edf2f7 !important; /* 👈 边框颜色更柔和 */
...
th {
  ...
  font-weight: bold !important;   /* 👈 改为标准加粗 */
  font-size: 13px !important;     /* 👈 缩小至 13px */
  text-align: center !important;
  white-space: nowrap !important;
}

td {
  color: #334155 !important;      /* 👈 颜色加深 */
  font-size: 13px !important;     /* 👈 缩小至 13px */
  text-align: center !important;
}
...
/* 复选框强对齐 */
.col-check { width: 40px !important; }
.col-index { width: 60px !important; }
...
.text-btn {
  ...
  color: #2a3f7a !important;
  font-size: 14px !important;
  font-weight: 400 !important;       /* 👈 1. 取消加粗，恢复和报价管理一致的常规粗细 */
  font-family: inherit !important;   /* 👈 2. 核心修复：强制 button 继承外部的漂亮字体 */
  cursor: pointer !important;
  padding: 0 !important;
  user-select: none !important;      /* 👈 3. 防选中，对齐报价管理的 span 行为 */
}
...
.icon-action:hover { opacity: 0.8 !important; }
.icon-action:disabled { opacity: 0.5 !important; cursor: not-allowed !important; }
...
.total-text { margin-right: 15px !important; }
...
.pager-arrow:hover:not(.disabled) { color: #2a3f7a !important; }
.pager-arrow.disabled { color: #c0c4cc !important; cursor: not-allowed !important; }
.pager-num.active { background-color: #2a3f7a !important; color: #fff !important; }

/* ==========================================
   5. 弹窗与表单 (保持编辑能力)
   ========================================== */
.modal-overlay {
  position: fixed !important; inset: 0 !important; background: rgba(15, 23, 42, 0.45) !important;
  z-index: 3000 !important; display: flex !important; align-items: center !important; justify-content: center !important;
}
.edit-modal, .preview-modal {
  background: #fff !important; border-radius: 18px !important; max-height: 90vh !important; overflow: hidden !important;
  display: flex !important; flex-direction: column !important; width: 1040px !important; max-width: 95vw !important;
  box-shadow: 0 24px 70px rgba(0, 0, 0, 0.24) !important;
}
.modal-header {
  display: flex !important; justify-content: space-between !important; align-items: center !important;
  padding: 24px 26px !important; border-bottom: 1px solid #edf2f7 !important;
}
.modal-header h3 {
  margin: 0 !important; font-size: 20px !important; color: #2a3f7a !important; font-weight: 800 !important;
  border-left: 4px solid #2a3f7a !important; padding-left: 10px !important; line-height: 1.1 !important;
}
.modal-close { background: none !important; border: none !important; font-size: 26px !important; color: #909399 !important; cursor: pointer !important; padding: 0 4px !important; }
.modal-close:hover { color: #f56c6c !important; }
.modal-body { padding: 26px !important; overflow-y: auto !important; flex: 1 !important; display: flex !important; flex-direction: column !important; gap: 24px !important; background: #f8fafc !important; }
.modal-footer { display: flex !important; justify-content: flex-end !important; gap: 12px !important; padding: 18px 26px 24px !important; border-top: 1px solid #edf2f7 !important; background: #fff !important;}

.form-card { background: #fff !important; border-radius: 12px !important; border: 1px solid #ebeef5 !important; padding: 20px !important; }
.form-card h4 { font-size: 17px !important; font-weight: 800 !important; color: #2a3f7a !important; border-left: 3px solid #2a3f7a !important; padding-left: 10px !important; margin: 0 0 16px 0 !important; line-height: 1.1 !important; }
.grid-two { display: grid !important; grid-template-columns: repeat(2, 1fr) !important; gap: 16px !important; }
.grid-one { display: grid !important; grid-template-columns: 1fr !important; gap: 16px !important; }
.field { display: flex !important; flex-direction: column !important; gap: 8px !important; }
.field span { font-size: 13px !important; font-weight: 800 !important; color: #6b7a99 !important; }
.field input, .field textarea { padding: 8px 12px !important; font-size: 14px !important; border: 1px solid #dfe7f2 !important; border-radius: 10px !important; outline: none !important; font-family: inherit !important; width: 100% !important; box-sizing: border-box !important; background: #f8fafd !important; color: #1e293b !important; }
.field input:focus, .field textarea:focus { border-color: #2a3f7a !important; background: #fff !important; }
.field textarea { min-height: 80px !important; resize: vertical !important; }

/* 文档预览专属 */
.preview-body { background: #f8fafc !important; }
.document-paper { max-width: 820px !important; margin: 0 auto !important; background: #fff !important; border: 1px solid #e2e8f0 !important; border-radius: 8px !important; padding: 40px !important; box-shadow: 0 8px 20px rgba(15, 23, 42, 0.05) !important; }
.cover-section { text-align: center !important; padding-bottom: 20px !important; margin-bottom: 20px !important; border-bottom: 1px dashed #d9e2ef !important; }
.cover-section h1 { margin: 0 0 16px !important; font-size: 28px !important; color: #1e293b !important; }
.doc-section { padding: 14px 0 !important; border-bottom: 1px solid #eef2f7 !important; }
.doc-section h2 { margin: 0 0 10px !important; color: #2a3f7a !important; font-size: 16px !important; }
.fade-enter-active, .fade-leave-active { transition: opacity 0.2s ease !important; }
...
.cb-wrap input { display: none !important; }
.cb-icon {
...
.bold-scheme {
  ...
  color: #2a3f7a !important;     /* 稍微加深颜色，使其更醒目（可选） */
}
