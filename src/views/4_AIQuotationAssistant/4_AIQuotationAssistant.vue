<template>
    ...
    <div v-if="!showForm && !showDetail" class="list-view">
      <div class="action-bar">
        ...
        <div class="bar-actions">
          ...
          <input v-model.trim="searchText" type="text" class="keyword-input search-box" placeholder="搜索委托单编号、报价编号或标题..."
        ...
        <table class="quotation-table">
          ...
          <tbody>
            <tr v-for="(item, index) in paginatedData" :key="item.id"
              ...
              <td>
                <span class="quote-no" style="color: #2a3f7a; font-weight: bold;">{{ item.orderNo || '--' }}</span>
              </td>
              <td>
      ...
      <div class="modal-content form-modal">
        ...
        <div class="modal-body">
          <div class="form-section">
            ...
            <div class="form-grid">
              <div class="form-item">
                <label class="form-label required">委托单编号</label>
                <div class="input-with-btn">
                  <input class="form-input" v-model="form.orderNo" placeholder="请选择委托单" readonly
                    style="cursor: pointer; background-color: #f8fafc;" @click="!isView && openOrderSelector()">
                  <button v-if="!isView" class="btn btn-primary select-btn" type="button"
                    @click="openOrderSelector">选择</button>
                </div>
              </div>
              <div class="form-item">
              ...
              <table class="item-table">
                ...
                <tbody>
                  <tr v-for="(item, idx) in form.items" :key="idx">
                    ...
                    <td class="col-pname">
                      <input v-if="item.editable" class="form-input" v-model="item.name" placeholder="自定义项目"
                        :readonly="isView">
                      <span v-else>{{ item.name }}</span>
                    </td>
                    <td class="col-price">
                      <input v-if="item.editable" class="form-input price-input" type="number"
                        v-model.number="item.unitPrice" :readonly="isView">
                      <span v-else>{{ item.unitPrice }}</span>
                    </td>
                    <td class="col-area">
                      <input v-if="!isView" class="form-input area-input" type="number" v-model.number="item.area"
                        placeholder="0" min="0">
      ...
      <div class="preview-modal-container" :class="{ 'is-fullscreen': isPreviewFullscreen }">
        <div class="preview-modal-header">
          ...
          <div class="preview-header-actions">
            <button class="preview-icon-btn" @click="togglePreviewFullscreen"
              :title="isPreviewFullscreen ? '还原' : '最大化'">
        ...
        <div class="preview-modal-body">
          <iframe v-if="currentPreviewItem" :src="previewUrl" width="100%" height="100%" frameborder="0"
            style="border: none; display: block; background-color: #525659;"></iframe>
        </div>
  ...
  <!-- 委托单选择弹窗 -->
  <div v-if="showOrderSelectModal" class="modal-overlay" style="z-index: 3000;">
    <div class="modal-content form-modal" style="width: 750px;">
      <div class="modal-header">
        <h3>选择委托单</h3>
        <button class="btn-close" @click="closeOrderSelector">×</button>
      </div>
      <div class="modal-body">
...
for (let i = 1; i <= 15; i++) {
  ...
  mockDataList.push({
    ...
    orderNo: `WT-2026-${String(i).padStart(3, '0')}`,
...
function selectOrderForQuote(order) {
  form.value.orderNo = order.orderNo;
  // 如果你需要自动带出联系人或项目名称，也可以在这里加
  closeOrderSelector();
}
...
.quotation-table tbody tr.selected {
